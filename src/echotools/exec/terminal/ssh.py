    ...
    async def start(self, cols: int = 80, rows: int = 24) -> bool:
        ...
        try:
            ...
            connect_kwargs = _build_ssh_connect_kwargs(
                self._host, self._port, self._username, self._password, self._key_data, pkey
            )
            ...
            self._ssh_channel = await _open_ssh_shell(self._ssh_client, cols, rows)
...
async def _open_ssh_shell(ssh_client: Any, cols: int, rows: int) -> Any:
