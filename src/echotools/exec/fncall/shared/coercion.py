...
def _try_parse_json_list(raw: str) -> Optional[list]:
    stripped = raw.strip()
...
def _try_parse_json_dict(raw: str) -> Optional[dict]:
    stripped = raw.strip()
    if not stripped.startswith("{"):
        return None
    try:
...
def _coerce_to_array(raw: str, parsed: Any, item_schema: Dict[str, Any]) -> Any:
    result_list = parsed if isinstance(parsed, list) else None
    if result_list is None and isinstance(parsed, str):
        result_list = _try_parse_json_list(parsed)
...
def _coerce_to_object(raw: str, parsed: Any, schema: Dict[str, Any]) -> Any:
    ...
    if result_dict is None and isinstance(parsed, str):
        result_dict = _try_parse_json_dict(parsed)
    if result_dict is None:
        result_dict = _try_parse_json_dict(raw)
    ...
    for k, v in result_dict.items():
        ...
        if isinstance(field_schema, dict) and field_schema:
            v_raw = v if isinstance(v, str) else json.dumps(v, ensure_ascii=False)
