...
from echotools.errors.http import ForbiddenError, StreamError
from echotools.events import Event, EventBus
from echotools.io import atomic_write_text, read_text_if_exists
from echotools.keys import KeyPool, KeyState
from echotools.lifecycle import LifecycleManager
from echotools.logger.manager import LoggerManager, _LogFormatter, _supports_color
...
from echotools.web import stats as stats_mod
...
def test_io_atomic_write_permission_retry(tmp_path: Path) -> None:
    ...
    with patch("echotools.io.io_utils.os.replace", side_effect=fake_replace):
...
def test_io_atomic_write_fallback_and_read(tmp_path: Path) -> None:
    ...
    with patch("echotools.io.io_utils.os.replace", side_effect=always_fail):
...
def test_proxy_selector_edge_cases(tmp_path: Path) -> None:
    ...
    with patch("echotools.dispatch.proxy_selector.atomic_write_text", side_effect=OSError("x")):
